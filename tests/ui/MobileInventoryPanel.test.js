...
it('supports tap categories, exact-index actions, live server updates and shortcut confirmation without losing selection', () => {
 ...
 click('设置快捷槽'); expect(body.querySelector('select').value).toBe('3'); body.querySelector('select').value = '35'; panel.update(); expect(body.querySelector('select').value).toBe('35'); click('确认设置'); expect(actions.bind).toHaveBeenCalledExactlyOnceWith(2, 501, 35);
