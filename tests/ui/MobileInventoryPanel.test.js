...
it('supports tap categories, exact-index actions, live server updates and shortcut confirmation without losing selection', () => {
 ...
 const actions = { snapshot: () => items, act: vi.fn(() => '已发送'), bind: vi.fn(() => true), shortcuts: () => ({ page: 1, pages: 8, total: 36, slots: [{ index: 5 }] }), slotName: () => '旧技能' };
 ...
 click('设置快捷槽'); expect(body.querySelector('[aria-label="目标快捷槽"]').value).toBe('5'); body.querySelector('[aria-label="目标快捷槽"]').value = '35'; panel.update(); expect(body.querySelector('[aria-label="目标快捷槽"]').value).toBe('35'); click('确认设置'); expect(actions.bind).toHaveBeenCalledExactlyOnceWith(2, 501, 35);
