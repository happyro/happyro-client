...
it('keeps configured species when it is absent nearby and saves multiple skills independently of shortcuts', () => {
	...
	const actions = { snapshot: () => ({ species: [{ id: 1002, name: '波利' }], skills: [5] }), targets: () => [{ species: 1003, name: '土波利' }], skills: () => [{ id: 5, name: '狂击', level: 3 }, { id: 19, name: '火箭术', level: 2, reason: 'SP 不足' }], configure: vi.fn(), close: vi.fn() };
	...
	expect(actions.configure).toHaveBeenLastCalledWith([{ id: 1002, name: '波利' }, { id: 1003, name: '土波利' }], [5, 19]);
	...
	expect(actions.configure).toHaveBeenLastCalledWith([], [5, 19]); expect(actions.close).toHaveBeenCalledTimes(2);
});
