...
it('selects real monsters and casts the learned level at the live GID', () => {
	const controller = createGameAutoCombat(() => true); controller.configure({ id: 1002, name: '波利' }, [5]); controller.start();
...
it('casts ground skills at the target position and excludes passive and friendly-only skills', () => {
	...
	controller.configure(null, [20]); controller.start(); expect(state.ground).toHaveBeenCalledExactlyOnceWith(20, 2, 2, 0);
});
it.each(['cooldown', 'sp'])('falls back to ordinary attacks when blocked by %s', reason => {
	...
	const controller = createGameAutoCombat(() => true); controller.configure(null, [5]); controller.start();
