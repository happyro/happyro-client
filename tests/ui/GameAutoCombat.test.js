...
it('casts ground skills at the target position and excludes passive and friendly-only skills', () => {
	...
	controller.configure([], [20]); controller.start(); expect(state.ground).toHaveBeenCalledExactlyOnceWith(20, 2, 2, 0);
});
it.each(['cooldown', 'sp'])('falls back to ordinary attacks when blocked by %s', reason => {
	...
	const controller = createGameAutoCombat(() => true); controller.configure([], [5]); controller.start();
