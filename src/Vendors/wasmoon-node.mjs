...
var exports = {};
	...
	var initWasmModule = (() => {
		var _scriptDir = (typeof document === 'undefined' && typeof location === 'undefined' ? "" : typeof document === 'undefined' ? location.href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.js', document.baseURI).href));
			...
			async function (moduleArg = {}) {
				...
				if (n) {
					if ("undefined" == typeof process || !process.release || "node" !== process.release.name) throw Error("not compiled for this environment (did you build to HTML and try to run it not on the web, or set ENVIRONMENT to something - like node - and run it someplace else - like on the web?)"); var la = process.versions.node, ma = la.split(".").slice(0, 3); ma = 1E4 * ma[0] + 100 * ma[1] + 1 * ma[2].split("-")[0]; if (16E4 > ma) throw Error("This emscripten-generated code requires node v16.0.0 (detected v" + la + ")"); const { createRequire: a } =
						await import('node:module'); var require$1 = a((typeof document === 'undefined' && typeof location === 'undefined' ? { pathToFileURL: () => ({ href: "" }) }.pathToFileURL(__filename).href : typeof document === 'undefined' ? location.href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.js', document.baseURI).href))), fs = require$1("fs"), na = require$1("path"); l ? r = na.dirname(r) + "/" : r = { fileURLToPath: () => "" }.fileURLToPath(new URL("./", (typeof document === 'undefined' && typeof location === 'undefined' ? { pathToFileURL: () => ({ href: "" }) }.pathToFileURL(__filename).href : typeof document === 'undefined' ? location.href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.js', document.baseURI).href)))); ia = (b, c) => { b = oa(b) ? new URL(b) : na.normalize(b); return fs.readFileSync(b, c ? void 0 : "utf8") }; ka = b => { b = ia(b, !0); b.buffer || (b = new Uint8Array(b)); u(b.buffer); return b }; ja = (b, c, d, f = !0) => { b = oa(b) ? new URL(b) : na.normalize(b); fs.readFile(b, f ? void 0 : "utf8", (h, m) => { h ? d(h) : c(f ? m.buffer : m); }); }; !e.thisProgram &&
