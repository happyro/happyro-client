...
import { openGameEnchant, finishGameEnchant } from 'UI/Game/GameEnchant.js';
import {
	getSlotValue,
	setSlotValue,
	getBaseSlotCount,
		...
		if (EnchantState.action === 'upgrade') {
			...
			if (upgradeCandidates.length) {
				...
				if (selectedEntry) {
					...
					if (ur) {
						ur.textContent = '结果：' + (selectedEntry.entry.result ? selectedEntry.entry.result.base : '');
					}
